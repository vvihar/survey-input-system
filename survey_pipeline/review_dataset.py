...
def _make_review_dataset_one(
    ...
    for booklet_index in range(n_booklets):
        ...
        rid_crop = crop_by_rect_pt(meta_scan_img, BOOKLET_META_RECT_BY_KIND["respondent_id"], dpi=dpi, pad_px=12)
        ver_crop = crop_by_rect_pt(meta_scan_img, BOOKLET_META_RECT_BY_KIND["version"], dpi=dpi, pad_px=12)
...
def make_review_dataset(
    ...
    rows = _make_review_dataset_one(answered_pdf, template_pdf, layout, initial_answers, workdir, dpi)
...
def make_review_datasets(
    ...
    for pdf in answered_pdfs:
        ...
        partial_answers: dict[str, Any] = {
            "source_pdf": sp,
            "template_pdf": merged_initial_answers.get("template_pdf", str(template_pdf)),
            "dpi": merged_initial_answers.get("dpi", dpi),
            "booklets": pdf_booklets,
        }
        rows = _make_review_dataset_one(pdf, template_pdf, layout, partial_answers, workdir, dpi)
    ...
    write_json(workdir / "layout.json", layout)
    write_jsonl(workdir / "review_items.jsonl", all_rows)
    return manifest


def update_booklet_meta(initial_answers: dict[str, Any], booklet_index: int, respondent_id: str, version: str) -> dict[str, Any]:
